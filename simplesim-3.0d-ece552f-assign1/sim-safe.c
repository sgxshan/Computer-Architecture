...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_counter(sdb, "sim_num_RAW_stall_q1",
		   "total number of RAW stalls (q1)",
		   &sim_num_RAW_stall_q1, sim_num_RAW_stall_q1, NULL);

  stat_reg_counter(sdb, "sim_num_RAW_stall_q2",
		   "total number of RAW stalls(q2)",
		   &sim_num_RAW_stall_q2, sim_num_RAW_stall_q2, NULL);


  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
		   "CPI from RAW hazard (q1)",
		   "(sim_num_insn + sim_num_RAW_stall_q1) / sim_num_insn" /* ECE552 - MUST ADD YOUR FORMULA */, NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
		   "CPI from RAW hazard (q2)",
		   "(sim_num_insn + sim_num_RAW_stall_q2) / sim_num_insn" /* ECE552 - MUST ADD YOUR FORMULA */, NULL);
...
#include "machine.def"
	...
	/*ECE552 Assignment 1 Q1 - BEGIN CODE*/
	int i = 0;
	int max_difference = 0;
	int difference = 0; 
	for(i =0; i<3; i++ ){	 		
		difference = reg_ready_q1[r_in[i]] - sim_num_insn;
		if(r_in[i]!=DNA&&difference > 0){
			reg_ready_q1[r_in[i]] -= difference;	//set register be available due to stall
			if(max_difference < difference)
				max_difference = difference;		//when instruction needs 2 cycles stall and 1 cycle stall, choose 2 cycles stall
		}
	}
	if(max_difference == 2){
		sim_num_RAW_hazard_q1 ++;		
	  	sim_num_RAW_stall_q1 += 2;		//two cycles stall condition
	}
	if (max_difference == 1){
        sim_num_RAW_hazard_q1 ++;
        sim_num_RAW_stall_q1 ++;		//one cycle stall condition	
	}
    if(r_out[0]!=DNA)
        reg_ready_q1[r_out[0]] = sim_num_insn + 3;
    if(r_out[1]!=DNA)
        reg_ready_q1[r_out[1]] = sim_num_insn + 3;
	/*ECE552 Assignment 1 Q1 - END CODE*/

        
    /* ECE552 Assignment 1 Q2 - BEGIN CODE */  	
	max_difference = 0;
	difference = 0; 
    for(i =0; i<3; i++ ){
		difference = reg_ready_q2[r_in[i]] - sim_num_insn;
		if(r_in[i]!=DNA&&difference > 0){
			//corner case of sotre instruction: no hazard
    		if((i==0) && (MD_OP_FLAGS(op)&F_MEM)&& (MD_OP_FLAGS(op)&F_STORE)){
        		continue;				
        	}
			reg_ready_q2[r_in[i]] -= difference;	//set register be available due to stall
			if(max_difference <= difference)
				max_difference =  difference;		//when instruction needs 2 cycles stall and 1 cycle stall, choose 2 cycles stall
		}
    }
    ...
    if(max_difference == 2){
        ...
        sim_num_RAW_stall_q2 += 2;		//two cycles stall condition
	}
	if (max_difference == 1){
        sim_num_RAW_hazard_q2 ++;
        sim_num_RAW_stall_q2 ++;		//one cycle stall condition	
	}
    ...
    /*ECE552 Assignment 1 Q2 - END CODE*/
