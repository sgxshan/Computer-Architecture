...
cp->blk_mask = bsize-1;
  ...
  cp->data = (byte_t *)calloc(nsets * assoc,
	...
	/* ECE552 Assignment 4 - BEGIN CODE*/
	/*initialize the reference prediction table*/
	if(prefetch_type>2){
		struct rpt_t *rpt = calloc(1, sizeof(struct rpt_t));
		rpt->tag = calloc(prefetch_type, sizeof(md_addr_t));
		rpt->prev_addr = calloc(prefetch_type, sizeof(md_addr_t));
		rpt->stride = calloc(prefetch_type, sizeof(md_addr_t));
		rpt->state = calloc(prefetch_type, sizeof(unsigned int));
		rpt->sign = calloc(prefetch_type, sizeof(bool));
		cp->rpt = rpt;
	}
	...
	if(prefetch_type == 2){
		struct ghb_t *ghb = calloc(1, sizeof(struct ghb_t));
		ghb->buffer = calloc(buffer_size, sizeof(md_addr_t));
		ghb->size = 0;
		ghb->head = 0;
		ghb->tail = 0;
		cp->ghb = ghb;
...
void next_line_prefetcher(struct cache_t *cp, md_addr_t addr) {
	...
	if(!cache_probe(cp, block_addr))	
		cache_access(cp,Read,block_addr,NULL,cp->bsize,NULL,NULL,NULL,1);
	
}

int ghb_search(struct ghb_t *ghb, md_addr_t addr){
	for(int i = 0; i<(ghb->size); i++){	
		if(ghb->buffer[i] == addr)
			return i;
	}
	return -1;
}

void ghb_insert(struct ghb_t *ghb, md_addr_t addr){
	int idx = ghb->tail;
	ghb->buffer[idx] = addr;
	ghb->tail++;
	ghb->tail = (ghb->tail)%buffer_size;
	ghb->size = ghb->size+1>buffer_size ? buffer_size : ghb->size+1;
}

md_addr_t ghb_next_addr(struct ghb_t *ghb, md_addr_t addr){
	int idx = ghb_search(ghb, addr);

	if(idx==buffer_size-1)
		return ghb->buffer[0];

	if(idx+1<=ghb->size-1)
		return ghb->buffer[idx+1];
	else
		return 0;
}
...
void open_ended_prefetcher(struct cache_t *cp, md_addr_t addr) {
	md_addr_t next_addr = 0;
	if(cp->miss_data){
		if(ghb_search(cp->ghb, addr)==-1){
			ghb_insert(cp->ghb, addr);
		}
		else{
			next_addr = ghb_next_addr(cp->ghb, addr);
		}
	}		
	if(next_addr!=0){
		md_addr_t tag = CACHE_TAG(cp, next_addr);
...
void state_exchange(struct rpt_t *rpt, int idx, bool same){
    switch (rpt->state[idx]){
		case init:
			if(same)
				rpt->state[idx] = steady;
			else
				rpt->state[idx] = transient;  
			break;
        case transient:
			if(same)
				rpt->state[idx]= steady;  
			else
				rpt->state[idx] = no_pred;  			
			break;
        case steady:
			if(!same)
				rpt->state[idx] = init;  			
...
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
	unsigned int offset = log_base2(sizeof(md_inst_t));
	unsigned int prefetch_type = cp->prefetch_type;
	unsigned int idx_mask = prefetch_type-1;
	unsigned int idx = idx_mask&(get_PC()>>offset);
	md_addr_t rpt_tag = get_PC()>>(log_base2(prefetch_type)+offset);
	bool same = true;	
	struct rpt_t *rpt = cp->rpt;
	if(rpt->tag[idx] != rpt_tag){
		rpt->tag[idx] = rpt_tag; 
		rpt->prev_addr[idx] = addr; 
		rpt->stride[idx] = 0;
		rpt->state[idx] = init;
		rpt->sign[idx] = true;
	}
		...
		md_addr_t diff;
		bool sign;
		if(addr > rpt->prev_addr[idx]){
			sign = true;
			diff = addr - rpt->prev_addr[idx];
		}	
		else{
			sign = false;
			diff = rpt->prev_addr[idx] - addr;
		}

		if(diff == rpt->stride[idx]&&sign==rpt->sign[idx]){
			same = true;
		}
		else{
			same = false;
		...
		rpt->stride[idx] = diff;
		rpt->sign[idx] = sign;
		state_exchange(rpt, idx, same);
	    //printf("%d,%d\n",idx,rpt->state[idx]);
		if(rpt->state[idx]!=no_pred){
			md_addr_t next_addr;
			if(sign==true)
				next_addr = addr + rpt->stride[idx];
			else
				next_addr = addr - rpt->stride[idx];
			md_addr_t tag = CACHE_TAG(cp, next_addr);
			md_addr_t set = CACHE_SET(cp, next_addr);
